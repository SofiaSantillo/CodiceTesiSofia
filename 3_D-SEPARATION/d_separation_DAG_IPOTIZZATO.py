...
# --- Configurazione ---
log_file = "3_D-SEPARATION/_logs/paths_DAG_IPOTIZZATO.log"
data_file = "Data_Droplet/seed_Binning_ordinato.csv"   # dataset discretizzato, tutte variabili intere
...
def conditional_mutual_information(x, y, z=None):
    ...
    if z is not None and len(z) > 0:
        z = [np.asarray(col).ravel() for col in z]  # tutti 1D
    ...
    for path in paths:
        ...
        for r in range(len(intermediates) + 1):
            for cond_set in combinations(intermediates, r):
                if cond_set:
                    # Allinea X, Y e Z rimuovendo NaN
                    combined = pd.concat([df[x_col], df[y_col]] + [df[col] for col in cond_set], axis=1).dropna()
                    x_vals_clean = combined[x_col].values  # 1D
                    y_vals_clean = combined[y_col].values  # 1D
                    z_vals_clean = [combined[col].values for col in cond_set]  # ogni Z è 1D
                    # Calcolo CMI manuale
...
if __name__ == "__main__":
    ...
    cmi_df = compute_cmi_paths(paths, df, x_col="PEG", y_col="CHOL")
    print(cmi_df)
    cmi_df.to_csv("3_D-SEPARATION/_logs/cmi_DAG_IPOTIZZATO.log", index=False)
