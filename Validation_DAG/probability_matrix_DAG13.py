...
def creation_dataset(log_path, dataset_path, DAG):
    ...
    df_new = df_cleaned.drop(columns=["PDI", "SIZE", "FRR", "PDI_TS"])
    output_file = os.path.join('Data_DAG', f'new_dataset_{DAG}.csv')
    df_new.to_csv(output_file, index=False)
    logger.info(f"Dataset con size e pdi categorizzati:\n{df_new}\n")

    # Plotta istogrammi
    plot_vars = ['FRR_CAT', 'SIZE_CAT', 'PDI_CAT']
...
def calcolo_matrice_probabilita(N, df, x, y):
    ...
    for pdi in x_values:
        row = [pdi]
        for size in y_values:
            prob = round(count.get((pdi, size), 0) / N, 4) #round(...,4) arrotonda a 4 cifre decimali
            row.append(prob)
        matrix.append(row)
...
def calcolo_matrice_probabilita_congiunta_totale(N, df, x, y, z):
    ...
    size_values = df[f"{y}"].unique()
    pdi_values = df[f"{z}"].unique()

    # Inizializza tutte le combinazioni possibili con count = 0
    for combo in product(frr_values, size_values, pdi_values):
        count_frr_size_pdi[combo] = 0
    ...
    for i in range(N):
        ...
        pdi = row[f"{z}"]
        key = (frr, size, pdi)
        count_frr_size_pdi[key] += 1

    # Calcola le probabilità
    prob_array = []
    for key, count in count_frr_size_pdi.items():
        prob = count / N
        prob_array.append([key, prob])  # [ (FRR, SIZE, PDI), Probabilità ]
...
def creation_matrix(df):
    ...
    matrix_frr_size, prob_marginale_size1, prob_marginale_frr1=calcolo_matrice_probabilita(N, df, x='FRR_CAT', y='SIZE_CAT')
    matrix_pdi_size, prob_marginale_size2, prob_marginale_pdi1=calcolo_matrice_probabilita(N, df, x='PDI_CAT', y='SIZE_CAT')
    matrix_frr_pdi, prob_marginale_pdi2, prob_marginale_frr2=calcolo_matrice_probabilita(N, df, x='FRR_CAT', y='PDI_CAT')

    probabilità_congiunta_tot=calcolo_matrice_probabilita_congiunta_totale(N, df, x='FRR_CAT', y='SIZE_CAT', z='PDI_CAT')

    prob_marginale_size1 = np.array(prob_marginale_size1, dtype=float)
    prob_marginale_size2 = np.array(prob_marginale_size2, dtype=float)
    ...
    if not np.allclose(prob_marginale_size1, prob_marginale_size2, atol=1e-3):
        logger.error("Le probabilita' marginali SIZE calcolate da diverse matrici non corrispondono!")
    else:
        logger.info("Le probabilita' marginali SIZE corrispondono.")
    ...
    return matrix_frr_size, matrix_pdi_size, matrix_frr_pdi, prob_marginale_frr, prob_marginale_size, probabilità_congiunta_tot




if __name__ == "__main__":
    df=creation_dataset(log_path, dataset_path, DAG="DAG1.3")
    creation_matrix(df)