...
# 2. Carica e discretizza il dataset
df = pd.read_csv("Data_DAG/Nodi_DAG1.csv")
df_disc = discretize(df, bins=4)

# Supponiamo che df sia il tuo DataFrame con colonne continue: 'FRR', 'SIZE', 'PDI'
...
f_pdi_given_size = df_disc.groupby('SIZE')['PDI'].value_counts(normalize=True).unstack().fillna(0)

# 5. Calcola la probabilità fattorizzata secondo il DAG
def get_fact_prob(row):
    frr, size, pdi = row['FRR'], row['SIZE'], row['PDI']
    
    # P(FRR), P(SIZE | FRR) e P(PDI | FRR, SIZE)
    p_frr = f_frr.get(frr, 0)
    p_size_given_frr = f_size_given_frr.loc[frr][size] if frr in f_size_given_frr.index else 0
    p_pdi_given_size = f_pdi_given_size.loc[size][pdi] if size in f_pdi_given_size.index else 0
    
    # Probabilità fattorizzata secondo il DAG: P(FRR) * P(SIZE | FRR) * P(PDI | FRR, SIZE)
    return p_frr * p_size_given_frr * p_pdi_given_size

joint['p_fact'] = joint.apply(get_fact_prob, axis=1)

# 6. Confronta la distribuzione congiunta con la fattorizzata
joint['ratio'] = joint['p_joint'] / joint['p_fact'].replace(0, np.nan)

# 7. Risultati
logging.info(f"RESULTS:\n{joint.sort_values('ratio', ascending=False).head(10)}")

# 8. Calcolo della percentuale di probabilità ben spiegata dal modello
lower, upper = 0.9, 1.1  # soglia di tolleranza per considerare il modello "corretto"
well_explained = joint[(joint['ratio'] >= lower) & (joint['ratio'] <= upper)]

# Somma delle p_joint delle righe ben spiegate
support_well_explained = well_explained['p_joint'].sum()

# Somma totale (per verifica, dovrebbe essere ≈ 1)
support_total = joint['p_joint'].sum()

# Percentuale di probabilità ben spiegata
