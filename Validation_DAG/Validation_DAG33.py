...
from probability_matrix_DAG33 import creation_matrix, creation_dataset
import numpy as np
import os
import json
...
def probabilita_fattorizzata(matrix_tfr_pdi, matrix_frr_tfr, prob_marginale_tfr, prob_empirica, logger):
    ...
    for j in range(1, matrix_frr_tfr.shape[1]):
        ...
        for i in range(matrix_frr_tfr.shape[0]):
            ...
            print(frr_value)
            print("elem tfr dato frr", matrix_frr_tfr[i,j], prob_marginale_tfr[j-1])
            ...
            for k in range(1,  matrix_tfr_pdi.shape[1]):
                pdi_value=k-1
                print(pdi_value)
                print("elem pdi dato frr", matrix_tfr_pdi[j-1,k], prob_marginale_tfr[j-1])
                ...
                print("---------", prob_marginale_tfr[j-1], p_codizionata_frr_dato_tfr, prob_condizionata_pdi)
    ...
    log_file = tabulate(log_data, headers=['FRR','PDI','TFR','P(TFR)','P(FRR|TFR)', 'P(PDI|TFR)', 'p. fattorizzata', 
...
def main():
    ...
    DAG = "DAG3.3"
    log_file = os.path.join(log_path, 'confronto_prob_DAG3.3.log')

    if not os.path.exists(log_path):
        os.makedirs(log_path)

    logger = logging.getLogger("confronto_prob_DAG3.3") 
    ...
    matrix_tfr_pdi, matrix_frr_tfr, p_marginale_tfr, prob_empirica = creation_matrix(df)
    ...
    empirica, fattorizzata=probabilita_fattorizzata(matrix_tfr_pdi, matrix_frr_tfr, p_marginale_tfr,  prob_empirica, logger)
