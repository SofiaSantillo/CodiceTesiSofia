...
from probability_matrix_DAG41 import creation_matrix, creation_dataset
import numpy as np
import os
import json
...
def probabilita_fattorizzata(matrix_tfr_size, matrix_size_pdi, prob_marginale_tfr, prob_marginale_size, prob_empirica, logger):
    ...
    for i in range(1, matrix_tfr_size.shape[1]):
        ...
        for j in range(matrix_tfr_size.shape[0]):
            ...
            for k in range(1,matrix_size_pdi.shape[1]): 
                pdi_value=k-1
                print("pdi:", pdi_value)
                print(matrix_size_pdi[j,k], prob_marginale_size[j])
                prob_condizionata_pdi= matrix_size_pdi[j,k]/prob_marginale_size[j]
                array_probabilita_codizionata_pdi_dato_size.append(prob_condizionata_pdi)
                prob_fatt = prob_marginale_tfr[i-1] * p_codizionata_size_dato_tfr * prob_condizionata_pdi
                ...
                log_data.append([tfr_value, size_value, pdi_value, prob_marginale_tfr[i-1],p_codizionata_size_dato_tfr, prob_condizionata_pdi, prob_fatt, probabilità_congiunta])

    # Scrive i dati nel log in formato tabellare
    log_file = tabulate(log_data, headers=['TFR','SIZE','PDI','P(TFR)','P(SIZE|TFR)', 'P(PDI|SIZE)', 'p. fattorizzata', 
...
def main():
    ...
    DAG = "DAG4.1"
    log_file = os.path.join(log_path, 'confronto_prob_DAG4.1.log')

    if not os.path.exists(log_path):
        os.makedirs(log_path)

    logger = logging.getLogger("confronto_prob_DAG4.1") 
    ...
    matrix_tfr_size, matrix_pdi_size, p_marginale_tfr, p_marginale_size, prob_empirica = creation_matrix(df)
    ...
    empirica, fattorizzata=probabilita_fattorizzata(matrix_tfr_size, matrix_pdi_size, p_marginale_tfr, p_marginale_size, prob_empirica, logger)
