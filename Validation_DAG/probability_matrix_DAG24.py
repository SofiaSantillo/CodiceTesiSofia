...
def calcolo_matrice_probabilita(N, df, x, y):
    ...
    for tfr in x_values:
        ...
        for size in y_values:
            prob = round(count.get((tfr, size), 0) / N, 4) #round(...,4) arrotonda a 4 cifre decimali
            row.append(prob)
        matrix.append(row)
...
def calcolo_matrice_probabilita_congiunta_totale(N, df, x, y, z):
    ...
    for combo in product(frr_values, size_values, tfr_values):
        count_frr_size_tfr[combo] = 0
    ...
    for i in range(N):
        ...
        tfr = row[f"{z}"]
        key = (frr, size, tfr)
        count_frr_size_tfr[key] += 1

    # Calcola le probabilità
    prob_array = []
    for key, count in count_frr_size_tfr.items():
        prob = count / N
        prob_array.append([key, prob])  # [ (FRR, SIZE, TFR), Probabilità ]
...
def creation_matrix(df):
    ...
    matrix_tfr_size, prob_marginale_size2, prob_marginale_tfr1=calcolo_matrice_probabilita(N, df, x='TFR_CAT', y='SIZE_CAT')
    matrix_frr_tfr, prob_marginale_tfr2, prob_marginale_frr2=calcolo_matrice_probabilita(N, df, x='FRR_CAT', y='TFR_CAT')

    probabilità_congiunta_tot=calcolo_matrice_probabilita_congiunta_totale(N, df, x='FRR_CAT', y='SIZE_CAT', z='TFR_CAT')
    ...
    return matrix_frr_size, matrix_frr_tfr, matrix_tfr_size, prob_marginale_frr, prob_marginale_size, prob_marginale_tfr, probabilità_congiunta_tot




if __name__ == "__main__":
    df=creation_dataset(log_path, dataset_path, DAG="DAG2.4")
    creation_matrix(df)