...
from probability_matrix_DAG24 import creation_matrix, creation_dataset
import numpy as np
import os
import json
...
def probabilita_fattorizzata(matrix_frr_size, matrix_frr_tfr, matrix_tfr_size, prob_marginale_frr, prob_marginale_size, prob_empirica, logger, prob_marginale_tfr):
    ...
    for i in range(matrix_frr_tfr.shape[0]):
        ...
        for j in range(1, matrix_frr_tfr.shape[1]):
            ...
            print("elem frr dato tfr", matrix_frr_tfr[i,j], prob_marginale_tfr[j-1])
            ...
            for k in range(1,  matrix_frr_size.shape[1]):
                ...
                print("elem size dato tfr", matrix_tfr_size[j-1,k], prob_marginale_tfr[j-1])
                prob_condizionata_size= matrix_tfr_size[j-1,k]/prob_marginale_tfr[j-1]
                array_probabilita_codizionata_size_dato_tfr.append(prob_condizionata_size)
                prob_fatt = prob_marginale_frr[i] * p_codizionata_frr_dato_tfr * prob_condizionata_size
                print("---------", prob_marginale_frr[i], p_codizionata_frr_dato_tfr, prob_condizionata_size)
                ...
                log_data.append([frr_value, size_value, tfr_value, prob_marginale_frr[i],p_codizionata_frr_dato_tfr, prob_condizionata_size, prob_fatt, probabilità_congiunta])

    # Scrive i dati nel log in formato tabellare
    log_file = tabulate(log_data, headers=['FRR','SIZE','TFR','P(TFR)','P(FRR|TFR)', 'P(SIZE|TFR)', 'p. fattorizzata', 
...
def main():
    ...
    DAG = "DAG2.4"
    log_file = os.path.join(log_path, 'confronto_prob_DAG2.4.log')

    if not os.path.exists(log_path):
        os.makedirs(log_path)

    logger = logging.getLogger("confronto_prob_DAG2.4") 
    ...
    matrix_frr_size, matrix_frr_tfr, matrix_tfr_size, p_marginale_frr, p_marginale_size, p_marginale_tfr, prob_empirica = creation_matrix(df)
    ...
    matrix_tfr_size = np.array(matrix_tfr_size, dtype=float)

    if not os.path.exists(log_path):
        os.makedirs(log_path)
    
    empirica, fattorizzata=probabilita_fattorizzata(matrix_frr_size, matrix_frr_tfr, matrix_tfr_size, p_marginale_frr, p_marginale_size, prob_empirica, logger, p_marginale_tfr)
