...
from probability_matrix_DAG21 import creation_matrix, creation_dataset
import numpy as np
import os
import json
...
def probabilita_fattorizzata(matrix_tfr_size, matrix_frr_tfr, prob_marginale_frr, prob_marginale_size, prob_empirica, logger, prob_marginale_tfr):
    ...
    for i in range(matrix_frr_tfr.shape[0]):
        ...
        for j in range(1, matrix_frr_tfr.shape[1]):
            ...
            print("elem prob tfr dato frr", matrix_frr_tfr[i,j], prob_marginale_frr[i])
            array_probabilita_codizionata_tfr_dato_frr.append(p_codizionata_tfr_dato_frr)

            for k in range(1,  matrix_tfr_size.shape[1]):
                size_value=k-1
                print("size: ", size_value)
                print("matrice tfr size")
                print(matrix_tfr_size[j-1,k], prob_marginale_tfr[j-1])
    ...
    log_file = tabulate(log_data, headers=['FRR','SIZE','TFR','P(FRR)','P(TFR|FRR)', 'P(SIZE|TFR)', 'p. fattorizzata', 
...
def main():
    ...
    DAG = "DAG2.1"
    log_file = os.path.join(log_path, 'confronto_prob_DAG2.1.log')

    if not os.path.exists(log_path):
        os.makedirs(log_path)

    logger = logging.getLogger("confronto_prob_DAG2.1") 
    ...
    matrix_tfr_size, matrix_frr_tfr, p_marginale_frr, p_marginale_size, p_marginale_tfr, prob_empirica = creation_matrix(df)
    ...
    empirica, fattorizzata=probabilita_fattorizzata(matrix_tfr_size, matrix_frr_tfr, p_marginale_frr, p_marginale_size, prob_empirica, logger, p_marginale_tfr)
