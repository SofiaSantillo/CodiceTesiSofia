...
def calcolo_matrice_probabilita_congiunta_totale(N, df, x, y, z):
    ...
    for combo in product(size_values, pdi_values, tfr_values):
        count_size_pdi_tfr[combo] = 0
    ...
    for i in range(N):
        ...
        tfr = row[f"{z}"]
        key = (size, pdi, tfr)
        count_size_pdi_tfr[key] += 1

    # Calcola le probabilità
    prob_array = []
    for key, count in count_size_pdi_tfr.items():
        prob = count / N
...
def creation_matrix(df):
    ...
    matrix_size_pdi, prob_marginale_pdi1, prob_marginale_size1=calcolo_matrice_probabilita(N, df, x='SIZE_CAT', y='PDI_CAT')
    matrix_tfr_pdi, prob_marginale_pdi2, prob_marginale_tfr1=calcolo_matrice_probabilita(N, df, x='TFR_CAT', y='PDI_CAT')
    matrix_size_tfr, prob_marginale_tfr2, prob_marginale_size2=calcolo_matrice_probabilita(N, df, x='SIZE_CAT', y='TFR_CAT')

    probabilità_congiunta_tot=calcolo_matrice_probabilita_congiunta_totale(N, df, x='SIZE_CAT', y='PDI_CAT', z='TFR_CAT')
    ...
    return matrix_size_tfr, matrix_size_pdi, prob_marginale_tfr, prob_marginale_size, probabilità_congiunta_tot




if __name__ == "__main__":
    df=creation_dataset(log_path, dataset_path, DAG="DAG4.1")
    creation_matrix(df)