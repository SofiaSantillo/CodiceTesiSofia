...
from probability_matrix_DAG22 import creation_matrix, creation_dataset
import numpy as np
import os
import json
    ...
    for j in range(1, matrix_frr_tfr.shape[1]):
        ...
        for i in range(matrix_frr_tfr.shape[0]):
            ...
            for k in range(1,  matrix_frr_size.shape[1]):
                size_value=k-1
                print(size_value)
                print("elem size dato frr", matrix_frr_size[i,k], prob_marginale_frr[i])
                ...
                print("---------", prob_marginale_frr[i], p_codizionata_tfr_dato_frr, prob_condizionata_size)
...
def main():
    ...
    DAG = "DAG2.2"
    log_file = os.path.join(log_path, 'confronto_prob_DAG2.2.log')

    if not os.path.exists(log_path):
        os.makedirs(log_path)

    logger = logging.getLogger("confronto_prob_DAG2.2") 
