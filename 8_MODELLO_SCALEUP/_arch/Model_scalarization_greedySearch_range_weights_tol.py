...
sys.stdout = Logger("8_MODELLO_SCALEUP/_log/Model_scalarization_greedSearch_range_weights_tol.log")

print( "\n-------------  START MODEL SCALARIZATION GREED SEARCH (scale factors + weights + TOL) ------------------")
...
def compute_metrics(y_true_size, y_true_pdi, y_pred_df):
    return {
        ...
        "R2_SIZE_tol": 1- mean_squared_error(y_true_size, y_pred_df["Refined_SIZE"])/100,
        "R2_PDI_tol": 1-mean_squared_error(y_true_pdi, y_pred_df["Refined_PDI"])/0.0025,
        "R2_mean_tol": (1- mean_squared_error(y_true_size, y_pred_df["Refined_SIZE"])/100 +
                    1-mean_squared_error(y_true_pdi, y_pred_df["Refined_PDI"])/0.0025) / 2,   
...
def apply_cluster_scale(X_col, cluster_factors, y_size, y_pdi, rf_model, xgb_size, xgb_pdi, num_epochs=5):
    ...
    for cluster, factors in cluster_factors.items():
        ...
        for f in factors:
            ...
            r2_mean = ((1 - mean_squared_error(y_size, y_pred_df["Refined_SIZE"])/100) + 
                       (1-mean_squared_error(y_pdi, y_pred_df["Refined_PDI"])/0.0025)) / 2
...
def optimize_global_weight(col_scaled, X_full, y_size, y_pdi, rf_model, xgb_size, xgb_pdi, num_epochs=5, weight_range=(0.5,2.0), step=0.01):
    best_weight = 1.0
    best_r2 = -np.inf
...
feature_weight= {"PEG", "ESM", "TFR", "HSPC", "CHOL", "FRR"}
for f in feature_weight:
    print("\n🔹 Ottimizzazione peso globale per PEG dopo cluster-specific scaling...")
    ...
    scaled_col_final, best_weight, best_r2 = optimize_global_weight(
        ...
        rf_model, xgb_size, xgb_pdi, num_epochs=5,
        weight_range=(0.5,2.0), step=0.01
    )
...
print("\n📊 Metriche finali sulle predizioni con PEG ottimizzato:")
for k, v in final_metrics.items():
    print(f"  {k}: {v:.4f}")

X_scaled_final.to_csv("_Data/dataset_ScaleUp_scaled_peg.csv", index=False)
print("\n✅ Dataset scalato con PEG ottimizzato salvato in '_Data/dataset_ScaleUp_scaled_peg.csv'")
