...
sys.stdout = Logger("8_MODELLO_SCALEUP/_log/Model_scalarization_greedSearch_range_weights(vincoli mse).log")
...
exclude_cols = ["AQUEOUS", "CHOL", "FRR", "HSPC"]
...
def apply_cluster_scale(X_col, cluster_factors, y_size, y_pdi, rf_model, xgb_size, xgb_pdi, num_epochs=5):
    ...
    for cluster, factors in cluster_factors.items():
        ...
        for f in factors:
            ...
            candidates.append((f, mse_size, mse_pdi))

        valid_candidates = [(f, mse_s, mse_p) for f, mse_s, mse_p in candidates if mse_s <= 30 and mse_p <= 0.001]

        if valid_candidates:
            best_factor, _, _ = min(valid_candidates, key=lambda x: x[1] + x[2])
        else:
            best_factor, _, _ = min(candidates, key=lambda x: x[1] + x[2])
            print(f"[!] Nessun fattore valido per cluster {cluster} in {X_col.name}, scelto migliore disponibile: {best_factor}")
...
def greedy_scale_search(X, y_size, y_pdi, scale_ranges, cols_to_scale):
    ...
    for col in cols_to_scale:
        ...
        if isinstance(scale_ranges[col], dict):
            ...
            print(f"[+] Feature {col}: cluster-specific scaling -> {best_factors[col]}")
        else:
            candidates = []
            ...
            if valid_candidates:
                ...
                best_factor, _, _ = min(candidates, key=lambda x: x[1] + x[2])
                print(f"[!] Nessun fattore valido per {col}, scelto migliore disponibile: {best_factor}")
            
            best_factors[col] = best_factor
            X_scaled[col] = X[col] * best_factor
            print(f"[+] Feature {col}: miglior fattore {best_factor}")
...
def optimize_global_weight(col_scaled, X_full, y_size, y_pdi, rf_model, xgb_size, xgb_pdi,
    ...
    for w in weights:
        ...
        mse_size = mean_squared_error(y_size, y_pred_df["Refined_SIZE"])
        mse_pdi = mean_squared_error(y_pdi, y_pred_df["Refined_PDI"])
        candidates.append((w, mse_size, mse_pdi))

    valid_candidates = [(w, mse_s, mse_p) for w, mse_s, mse_p in candidates if mse_s <= 30 and mse_p <= 0.001]

    if valid_candidates:
        best_weight, best_mse_size, best_mse_pdi = min(valid_candidates, key=lambda x: x[1] + x[2])
        best_score = best_mse_size + best_mse_pdi
    else:
        best_weight, best_mse_size, best_mse_pdi = min(candidates, key=lambda x: x[1] + x[2])
        best_score = best_mse_size + best_mse_pdi
        print(f"[!] Nessun peso valido trovato per {col_scaled.name}, scelto migliore disponibile: {best_weight}")

    col_final = col_scaled * best_weight
    return col_final, best_weight, best_score
