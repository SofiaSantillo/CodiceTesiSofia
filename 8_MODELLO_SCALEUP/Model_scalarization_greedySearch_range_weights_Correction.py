...
sys.stdout = Logger("8_MODELLO_SCALEUP/_log/Model_scalarization_greedSearch_range_weights_Corrections.log")
...
exclude_cols = ["AQUEOUS", "CHOL", "HSPC"]
...
def apply_cluster_scale(X_col, cluster_factors, y_size, y_pdi, rf_model, xgb_size, xgb_pdi, num_epochs=5):
    ...
    for cluster, factors in cluster_factors.items():
        ...
        for f in factors:
            ...
            r2_mean = (r2_score(y_size, y_pred_df["Refined_SIZE"]) + r2_score(y_pdi, y_pred_df["Refined_PDI"])) / 2
...
def greedy_scale_search(X, y_size, y_pdi, scale_ranges, cols_to_scale):
    ...
    for col in cols_to_scale:
        ...
        if isinstance(scale_ranges[col], dict):
            ...
            for f in factors:
                ...
                r2_mean = (r2_score(y_size, y_pred_df["Refined_SIZE"]) +
                           r2_score(y_pdi, y_pred_df["Refined_PDI"])) / 2
                r2_per_factor[f] = r2_mean
    ...
    final_pred_df = iterative_refinement_predict(X_scaled, rf_model, xgb_size, xgb_pdi, num_epochs=10)
    final_metrics = compute_metrics(y_size, y_pdi, final_pred_df)
    
    return best_factors, final_metrics

def optimize_global_weight(col_scaled, X_full, y_size, y_pdi, rf_model, xgb_size, xgb_pdi, num_epochs=5, weight_range=(0.1,3.0), step=0.01):
    best_weight = 1.0
    best_r2 = -np.inf
    ...
    return col_final, best_weight, best_r2
...
refined_pred = iterative_refinement_predict(X_val, rf_model, xgb_size, xgb_pdi, num_epochs=10)
...
print("\n📊 Metriche finali sulle predizioni:")
...
# === SIZE ===
axes[0].plot(y_val_size.values, label="True SIZE", marker="o", linestyle="-")
axes[0].plot(y_pred_final["Refined_SIZE"].values, label="Final Pred SIZE", marker="x", linestyle="--")
axes[0].set_title("SIZE: True vs Predicted vs Calibrated")
axes[0].set_xlabel("Sample Index")
axes[0].set_ylabel("SIZE Value")
axes[0].legend()
axes[0].grid(True)

# === PDI ===
axes[1].plot(y_val_pdi.values, label="True PDI", marker="o", linestyle="-")
axes[1].plot(y_pred_final["Refined_PDI"].values, label="Final Pred PDI", marker="x", linestyle="--")
axes[1].set_title("PDI: True vs Predicted vs Calibrated")
...
plt.tight_layout()
plt.savefig("8_MODELLO_SCALEUP/_plot/Model_scalarization_results.png", dpi=300)

plt.close()

# === Calibrazione SIZE ===
raw_size_pred = y_pred_final["Refined_SIZE"].values

# 1. Polynomial Regression
poly = PolynomialFeatures(degree=5, include_bias=False)
X_poly_size = poly.fit_transform(raw_size_pred.reshape(-1, 1))
poly_reg_size = LinearRegression().fit(X_poly_size, y_val_size)
poly_size_pred = poly_reg_size.predict(X_poly_size)
print("Polynomial Calibrated R² SIZE:", r2_score(y_val_size, poly_size_pred))

# 2. Quantile Mapping (Isotonic Regression)
iso_reg_size = IsotonicRegression(out_of_bounds="clip")
iso_reg_size.fit(raw_size_pred, y_val_size)
iso_size_pred = iso_reg_size.predict(raw_size_pred)
print("Quantile Calibrated R² SIZE:", r2_score(y_val_size, iso_size_pred))


# === Calibrazione PDI ===
raw_pdi_pred = y_pred_final["Refined_PDI"].values

# 1. Polynomial Regression
poly = PolynomialFeatures(degree=5, include_bias=False)
X_poly_pdi = poly.fit_transform(raw_pdi_pred.reshape(-1, 1))
poly_reg_pdi = LinearRegression().fit(X_poly_pdi, y_val_pdi)
poly_pdi_pred = poly_reg_pdi.predict(X_poly_pdi)
print("Polynomial Calibrated R² PDI:", r2_score(y_val_pdi, poly_pdi_pred))

# 2. Quantile Mapping (Isotonic Regression)
iso_reg_pdi = IsotonicRegression(out_of_bounds="clip")
iso_reg_pdi.fit(raw_pdi_pred, y_val_pdi)
iso_pdi_pred = iso_reg_pdi.predict(raw_pdi_pred)
print("Quantile Calibrated R² PDI:", r2_score(y_val_pdi, iso_pdi_pred))
...
axes[0].plot(y_val_size.values, label="True SIZE", marker="o", linestyle="-")
axes[0].plot(y_pred_final["Refined_SIZE"].values, label="Final Pred SIZE", marker="x", linestyle="--")
axes[0].plot(poly_size_pred, label="Poly Calibrated SIZE", marker="s", linestyle="--")
axes[0].plot(iso_size_pred, label="Isotonic Calibrated SIZE", marker="^", linestyle="--")
axes[0].set_title("SIZE: True vs Predicted vs Calibrated")
axes[0].set_xlabel("Sample Index")
axes[0].set_ylabel("SIZE Value")
axes[0].legend()
...
axes[1].plot(y_val_pdi.values, label="True PDI", marker="o", linestyle="-")
axes[1].plot(y_pred_final["Refined_PDI"].values, label="Final Pred PDI", marker="x", linestyle="--")
axes[1].plot(poly_pdi_pred, label="Poly Calibrated PDI", marker="s", linestyle="--")
axes[1].plot(iso_pdi_pred, label="Isotonic Calibrated PDI", marker="^", linestyle="--")
axes[1].set_title("PDI: True vs Predicted vs Calibrated")
axes[1].set_xlabel("Sample Index")
axes[1].set_ylabel("PDI Value")
axes[1].legend()
...
plt.savefig("8_MODELLO_SCALEUP/_plot/Model_scalarization_calibration_results.png", dpi=300)
