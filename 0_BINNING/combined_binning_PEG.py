...
if __name__ == "__main__":
    ...
    print(df['PEG'])
    k_bins_sqrt = binning_sqrt(df, 'PEG')
    ...
    k_bins_rice = binning_rice_rule(df, 'PEG')
    df['PEG_binned_rice'] = bin_variable_by_k(df, 'PEG', k_bins_rice)


    k_bins_ts = binning_terrel_scott(df, 'PEG')
    df['PEG_binned_ts'] = bin_variable_by_k(df, 'PEG', k_bins_ts)


    k_bins_fd = binning_freedman_diaconis(df, 'PEG')
    df['PEG_binned_fd'] = bin_variable_by_k(df, 'PEG', k_bins_fd)
    

    fig, axes = plt.subplots(2, 3, figsize=(18, 12))  # Due righe, tre colonne

    # Primo istogramma (basato su √n)
    df['PEG_binned_sqrt'].value_counts().sort_index().plot(kind='bar', ax=axes[0, 0], color='orange', edgecolor='black')
    axes[0, 0].set_title(f"Istogramma binned di PEG (k=⌈√n⌉)")
    axes[0, 0].set_xlabel("Intervalli")
    axes[0, 0].set_ylabel("Frequenza")
    axes[0, 0].tick_params(axis='x', rotation=45)

    # Secondo istogramma (basato sulla distribuzione dei dati)
    df['PEG_binned_distribution'].value_counts().sort_index().plot(kind='bar', ax=axes[0, 1], color='green', edgecolor='black')
    axes[0, 1].set_title(f"Istogramma binned di PEG (k=|max-min/h|)")
    axes[0, 1].set_xlabel("Intervalli")
    axes[0, 1].set_ylabel("Frequenza")
    axes[0, 1].tick_params(axis='x', rotation=45)

    # Terzo istogramma (basato sulla regola del riso)
    df['PEG_binned_rice'].value_counts().sort_index().plot(kind='bar', ax=axes[0, 2], color='yellow', edgecolor='black')
    axes[0, 2].set_title(f"Istogramma binned di PEG (Rice Rule)")
    axes[0, 2].set_xlabel("Intervalli")
    axes[0, 2].set_ylabel("Frequenza")
    axes[0, 2].tick_params(axis='x', rotation=45)

    # Quarto istogramma (basato sulla regola Terrell-Scott)
    df['PEG_binned_ts'].value_counts().sort_index().plot(kind='bar', ax=axes[1, 0], color='red', edgecolor='black')
    axes[1, 0].set_title(f"Istogramma binned di PEG (Terrell-Scott)")
    ...
    df['PEG_binned_fd'].value_counts().sort_index().plot(kind='bar', ax=axes[1, 1], color='blue', edgecolor='black')
    axes[1, 1].set_title(f"Istogramma binned di PEG (Freedman-Diaconis)")
    ...
    plt.tight_layout()
    plt.savefig("0_BINNING/Plot_binning/combined_histogram_PEG.png")
    plt.close()
