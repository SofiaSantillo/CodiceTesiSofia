...
dataset_path = 'Data_Droplet/seed.csv' 
...
def remove_cat_suffix(df):
    # Rinomina le colonne rimuovendo il suffisso "_cat"
...
def creation_dataset(log_path, dataset_path):
    ...
    df_new = df_cleaned.drop(columns=["PDI", "SIZE", "FRR", "TFR", "PDI_TS", "ESM", "HSPC", "CHOL", "CHOL_TS", "PEG"])

    df = remove_cat_suffix(df_new)

    output_file = os.path.join('Data_Droplet', f'seed_Binning.csv')
