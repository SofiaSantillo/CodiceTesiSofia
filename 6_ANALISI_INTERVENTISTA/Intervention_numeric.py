...
############ SCRIPT PER LE VARIABILI: CHOL, ESM, HSPC, PEG, TFR, FRR #############################
...
x_intervento = df[intervention_variable].unique()
i=0
for x in x_intervento:
    ...
    df_intervento_ML[intervention_variable] = x #Modifca rispetto a "intervention_categorical.py"
    ...
    for n in nodes:
        ...
        if delta_DAG * delta_ML >= 0 and diff_perc / 100 < tolleranza + abs(delta_diff / baseline_diff):
            ...
            shap_diff_df = pd.DataFrame(columns=['Feature', 'Importance_intervento', 'Importance_original', 'Diff', 'valutazione'])
