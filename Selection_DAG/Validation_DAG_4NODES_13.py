        ...
        for k in range(1, matrix_frr_tfr.shape[1]):
            ...
            for j in range(matrix_pdi_frr.shape[0]): 
                ...
                for n in range(matrix_size_pdi.shape[0]):
                    ...
                    prob_fatt = prob_marginale_frr[i] * p_codizionata_tfr_dato_frr * p_condizionata_pdi_dato_frr * prob_condizionata_size
                    ...
                    log_data.append([frr_value, size_value, pdi_value, tfr_value, prob_marginale_frr[i], p_codizionata_tfr_dato_frr, p_condizionata_pdi_dato_frr, prob_condizionata_size, prob_fatt, probabilità_congiunta])

    # Scrive i dati nel log in formato tabellare
    log_file = tabulate(log_data, headers=['FRR','SIZE','PDI','TFR', 'P(FRR)','P(TFR|FRR)','P(PDI|FRR)', 'P(SIZE|PDI)', 'p. fattorizzata', 
...
def main():
    ...
    DAG = "DAG_4NODES_13"
    log_file = os.path.join(log_path, 'confronto_prob_DAG_4NODES_13.log')

    if not os.path.exists(log_path):
        os.makedirs(log_path)

    logger = logging.getLogger("confronto_prob_DAG_4NODES_13") 
