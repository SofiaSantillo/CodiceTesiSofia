...
if __name__ == "__main__":
    file_path = "Data_DAG/Nodi_DAG1.csv"
    ...
    k_bins_sqrt = binning_sqrt(df, 'FRR')
    df['FRR_binned_sqrt'] = bin_variable_by_k(df, 'FRR', k_bins_sqrt)

    k_bins_distribution = binning_max_min(df, 'FRR', h=18)
    df['FRR_binned_distribution'] = bin_variable_by_k(df, 'FRR', k_bins_distribution)

    k_bins_rice = binning_rice_rule(df, 'FRR')
    df['FRR_binned_rice'] = bin_variable_by_k(df, 'FRR', k_bins_rice)

    k_bins_ts = binning_terrel_scott(df, 'FRR')
    df['FRR_binned_ts'] = bin_variable_by_k(df, 'FRR', k_bins_ts)

    k_bins_fd = binning_freedman_diaconis(df, 'FRR')
    ...
    axes[0, 0].set_title(f"Istogramma binned di FRR (k=⌈√n⌉)")
    axes[0, 0].set_xlabel("Intervalli")
    axes[0, 0].set_ylabel("Frequenza")
    axes[0, 0].tick_params(axis='x', rotation=45)

    # Secondo istogramma (basato sulla distribuzione dei dati)
    df['FRR_binned_distribution'].value_counts().sort_index().plot(kind='bar', ax=axes[0, 1], color='green', edgecolor='black')
    axes[0, 1].set_title(f"Istogramma binned di FRR (k=|max-min/h|)")
    axes[0, 1].set_xlabel("Intervalli")
    axes[0, 1].set_ylabel("Frequenza")
    axes[0, 1].tick_params(axis='x', rotation=45)

    # Terzo istogramma (basato sulla regola del riso)
    df['FRR_binned_rice'].value_counts().sort_index().plot(kind='bar', ax=axes[0, 2], color='yellow', edgecolor='black')
    axes[0, 2].set_title(f"Istogramma binned di FRR (Rice Rule)")
    axes[0, 2].set_xlabel("Intervalli")
    axes[0, 2].set_ylabel("Frequenza")
    axes[0, 2].tick_params(axis='x', rotation=45)

    # Quarto istogramma (basato sulla regola Terrell-Scott)
    df['FRR_binned_ts'].value_counts().sort_index().plot(kind='bar', ax=axes[1, 0], color='red', edgecolor='black')
    axes[1, 0].set_title(f"Istogramma binned di FRR (Terrell-Scott)")
    ...
    df['FRR_binned_fd'].value_counts().sort_index().plot(kind='bar', ax=axes[1, 1], color='blue', edgecolor='black')
    axes[1, 1].set_title(f"Istogramma binned di FRR (Freedman-Diaconis)")
    ...
    plt.tight_layout()
    plt.savefig("Studing_Binning/combined_histogram_FRR.png")
    plt.close()
