...
if __name__ == "__main__":
    ...
    k_bins_sqrt = binning_sqrt(df, 'PDI')
    df['PDI_binned_sqrt'] = bin_variable_by_k(df, 'PDI', k_bins_sqrt)

  
    k_bins_distribution = binning_max_min(df, 'PDI', h=0.1)
    df['PDI_binned_distribution'] = bin_variable_by_k(df, 'PDI', k_bins_distribution)

  
    k_bins_rice = binning_rice_rule(df, 'PDI')
    df['PDI_binned_rice'] = bin_variable_by_k(df, 'PDI', k_bins_rice)

   
    k_bins_ts = binning_terrel_scott(df, 'PDI')
    df['PDI_binned_ts'] = bin_variable_by_k(df, 'PDI', k_bins_ts)

   
    k_bins_fd = binning_freedman_diaconis(df, 'PDI')
    ...
    axes[0, 0].set_title(f"Istogramma binned di PDI (k=⌈√n⌉)")
    axes[0, 0].set_xlabel("Intervalli")
    axes[0, 0].set_ylabel("Frequenza")
    axes[0, 0].tick_params(axis='x', rotation=45)

    # Secondo istogramma (basato sulla distribuzione dei dati)
    df['PDI_binned_distribution'].value_counts().sort_index().plot(kind='bar', ax=axes[0, 1], color='green', edgecolor='black')
    axes[0, 1].set_title(f"Istogramma binned di PDI (k=|max-min/h|)")
    axes[0, 1].set_xlabel("Intervalli")
    axes[0, 1].set_ylabel("Frequenza")
    axes[0, 1].tick_params(axis='x', rotation=45)

    # Terzo istogramma (basato sulla regola del riso)
    df['PDI_binned_rice'].value_counts().sort_index().plot(kind='bar', ax=axes[0, 2], color='yellow', edgecolor='black')
    axes[0, 2].set_title(f"Istogramma binned di PDI (Rice Rule)")
    axes[0, 2].set_xlabel("Intervalli")
    axes[0, 2].set_ylabel("Frequenza")
    axes[0, 2].tick_params(axis='x', rotation=45)

    # Quarto istogramma (basato sulla regola Terrell-Scott)
    df['PDI_binned_ts'].value_counts().sort_index().plot(kind='bar', ax=axes[1, 0], color='red', edgecolor='black')
    axes[1, 0].set_title(f"Istogramma binned di PDI (Terrell-Scott)")
    ...
    df['PDI_binned_fd'].value_counts().sort_index().plot(kind='bar', ax=axes[1, 1], color='blue', edgecolor='black')
    axes[1, 1].set_title(f"Istogramma binned di PDI (Freedman-Diaconis)")
    ...
    plt.tight_layout()
    plt.savefig("Studing_Binning/combined_histogram_PDI.png")
    plt.close()
