...
def analyze_dag(dag):
    ...
    structure_file = "DAG_manuale/dag1_structure.txt"

    # Write to file
    with open(structure_file, "w", encoding="utf-8") as f:
        f.write("DAG1 STRUCTURE:\n\n\n")
        f.write("Collider (A → B ← C):\n")
...
# Function to build non-linear model
...
# Function to compare models and write results to file
def compare_models(dag, dataset, log_filename):
    ...
    with open(log_filename, "w") as log_file:
        log_file.write("Comparison between linear and non-linear models:\n\n")
        ...
        for node, (model, r2) in nonlinear_models.items():
            log_file.write(f"Node: {node}, Non-Linear Model: {model}, R²: {r2}\n")
        ...
        for node in dag.nodes:
            ...
            if linear_r2 and nonlinear_r2:
                if linear_r2 > nonlinear_r2:
                    log_file.write(f"\nFor node {node}, the linear model performs better with R²: {linear_r2} vs {nonlinear_r2}.")
                else:
                    log_file.write(f"\nFor node {node}, the non-linear model performs better with R²: {nonlinear_r2} vs {linear_r2}.")
            elif linear_r2:
                log_file.write(f"\nFor node {node}, only the linear model is available with R²: {linear_r2}.")
            elif nonlinear_r2:
                log_file.write(f"\nFor node {node}, only the non-linear model is available with R²: {nonlinear_r2}.")
    return
...
plt.savefig('_Plot/dag1_output.png', format='png')
plt.close()

# Analyze DAG structures
forks, colliders, chains, backdoor_paths = analyze_dag(dag)
# Compare the models
log_filename = '_Logs/eq_strutturate_dag1.log'
